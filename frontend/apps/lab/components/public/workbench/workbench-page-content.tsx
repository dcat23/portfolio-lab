...
import type { ActivityItem, Repo } from "@feature/lab-client/server"
...
export function WorkbenchPageContent({ wipItems, activityItems: recentActivity }: Props) {
  const [hoveredItem, setHoveredItem] = useState<string | null>(null)
  const [isVisible, setIsVisible] = useState(false);

  const avgProgress = useMemo(() => {
    if (wipItems.length === 0) return 100;
    return Math.round(wipItems.reduce((a, b) => a + b.progress, 0) / wipItems.length);
  }, [wipItems]);
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <div className="flex items-center gap-3 border-b border-border/50 bg-secondary/40 px-4 sm:px-5 py-3.5 sm:py-4">
                ...
                <span className="ml-4 font-mono text-xs text-muted-foreground truncate">~/devincatuns/active</span>
